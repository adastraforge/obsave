...
- Scope `drive` completo; paginación en listado de carpetas; toggle auto-sync ON/OFF.
- `FileStatusDecorator`: badges rojo/amarillo/verde para GitHub y Google Drive.
...
- **Routing:** conectado → panel; sin proveedor → selector con badges Conectado/Sin configurar.
- Sin Notice automático por falta de proveedor; leyendas técnicas OAuth eliminadas.
...
- **`GoogleDriveProvider`:** Notice con `error_description` de Google; `GoogleDriveAuthContext` para guardar y repintar UI.
...
- **`GoogleDriveProvider`:** logs PKCE, URL auth, puerto 42000, token request y `[ObSave Token Error Body]`.
...
- **`GoogleDriveProvider`:** `exchangeCodeForTokens` público; errores de token capturados con mensaje unificado.
- **`ObSaveSettingTab`:** spinner «Conectando…»; estado «Cuenta de Google Conectada (email)» tras auth.
...
- **`GoogleDriveLazyProvider`:** carga diferida de `GoogleDriveProvider` solo en OAuth/sync/botón conectar.
- **`runtimeBridge` / `localCallbackServer`:** `window.require('http')` dentro del flujo OAuth; errores capturados sin bloquear carga.
- **`main.ts`:** init defensivo; config GDrive pendiente sin módulos Node hasta acción del usuario.
...
└── ui/
    ...
    └── FileStatusDecorator.ts # Badges de estado en Explorador
```
